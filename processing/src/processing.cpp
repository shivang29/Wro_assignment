  ...
  try
  {
    ...
    if(!src.empty())
    {
      ...
      if (contours.size() != 0)
      {
        ...
        point_message.header.frame_id = camera_frame.c_str();
        point_message.point.x = (bounding_rectangle.br().x + bounding_rectangle.tl().x)/2 - (src.size().width)/2;
        point_message.point.y = ((float)src.size().height)/2 - (bounding_rectangle.br().y + bounding_rectangle.tl().y)/2;
       point_message.point.z = 1 + (
(known_width * focal_length) / (bounding_rectangle.br().x + bounding_rectangle.tl().x));

        ROS_INFO("point Location (x, y, z) = (%.2f, %.2f, %.2f)", point_message.point.x, point_message.point.y,point_message.point.z);

        coordinates_pub.publish(point_message);
      }
    }
  }
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("processing node initialized");

  // reading parameters from the paramter server
  ros::param::get("processing/focal_length", focal_length);
  ros::param::get("processing/known_width", known_width);
  ros::param::get("processing/low_b", low_bgr[0]);
  ros::param::get("processing/low_g", low_bgr[1]);
  ros::param::get("processing/low_r", low_bgr[2]);
  ros::param::get("processing/high_b", high_bgr[0]);
  ros::param::get("processing/high_g", high_bgr[1]);
  ros::param::get("processing/high_r", high_bgr[2]);

  ROS_INFO("Thresholding BGR with range: (%d, %d, %d) - (%d, %d, %d)", low_bgr[0], low_bgr[1], low_bgr[2], high_bgr[0], high_bgr[1], high_bgr[2]);

  // initializing publishers
  coordinates_pub = nh.advertise<geometry_msgs::PointStamped>("/processing/coordinates", 1000);
